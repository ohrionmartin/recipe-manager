 ...
 * Recipes Table class
 */
class Recipe_managerTableRecipe extends JTable
{
	/**
	...
	{
		parent::__construct('#__recipe_manager_recipe', 'id', $db);

		// [Interpretation 7724] Adding History Options
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_recipe_manager.recipe'));
	}	
	...
	/**
	 * Overload the store method for the Recipe table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New recipe. A recipe created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(JText::_('COM_RECIPE_MANAGER_RECIPE_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_recipe_manager.recipe.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_recipe_manager.recipe.'.(int) $this->$k;
	}
