 ...
 * Recipes Controller
 */
class Recipe_managerControllerRecipes extends JControllerAdmin
{
	/**
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_RECIPE_MANAGER_RECIPES';
	 ...
	 * @since   1.6
	 */
	public function getModel($name = 'Recipe', $prefix = 'Recipe_managerModel', $config = array('ignore_request' => true))
	{
	...
	public function exportData()
	{
		...
		if ($user->authorise('recipe.export', 'com_recipe_manager') && $user->authorise('core.export', 'com_recipe_manager'))
		{
			...
			if (Recipe_managerHelper::checkArray($data))
			{
				...
				Recipe_managerHelper::xls($data,'Recipes_'.$date->format('jS_F_Y'),'Recipes exported ('.$date->format('jS F, Y').')','recipes');
			}
		}
		// [Interpretation 15275] Redirect to the list screen with error.
		$message = JText::_('COM_RECIPE_MANAGER_EXPORT_FAILED');
		$this->setRedirect(JRoute::_('index.php?option=com_recipe_manager&view=recipes', false), $message, 'error');
		return;
	}
	...
	public function importData()
	{
		...
		$user = JFactory::getUser();
		if ($user->authorise('recipe.import', 'com_recipe_manager') && $user->authorise('core.import', 'com_recipe_manager'))
		{
			...
			if (Recipe_managerHelper::checkObject($headers))
			{
				...
				$session->set('recipe_VDM_IMPORTHEADERS', $headers);
				$session->set('backto_VDM_IMPORT', 'recipes');
				$session->set('dataType_VDM_IMPORTINTO', 'recipe');
				// [Interpretation 15327] Redirect to import view.
				$message = JText::_('COM_RECIPE_MANAGER_IMPORT_SELECT_FILE_FOR_RECIPES');
