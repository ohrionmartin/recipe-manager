...
class RecipemanagerViewRecipes extends JViewLegacy
{
	...
	function display($tpl = null)
	{
		if ($this->getLayout() !== 'modal')
		{
			...
			RecipemanagerHelper::addSubmenu('recipes');
		}
	...
	{
		JToolBarHelper::title(JText::_('COM_RECIPEMANAGER_RECIPES'), 'joomla');
		JHtmlSidebar::setAction('index.php?option=com_recipemanager&view=recipes');
		...
		if (RecipemanagerHelper::checkArray($this->items))
		{
			...
			{
				JToolBarHelper::custom('recipes.exportData', 'download', '', 'COM_RECIPEMANAGER_EXPORT_DATA', true);
			}
		}

		if ($this->canDo->get('core.import') && $this->canDo->get('recipe.import'))
		{
			JToolBarHelper::custom('recipes.importData', 'upload', '', 'COM_RECIPEMANAGER_IMPORT_DATA', false);
		}
		...
		// [Interpretation 18462] Category Filter.
		JHtmlSidebar::addFilter(
			JText::_('JOPTION_SELECT_CATEGORY'),
			'filter_category_id',
			JHtml::_('select.options', JHtml::_('category.options', 'com_recipemanager'), 'value', 'text', $this->state->get('filter.category_id'))
		);
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			// [Interpretation 18723] Category Batch selection.
			JHtmlBatch_::addListSelection(
				JText::_('COM_RECIPEMANAGER_KEEP_ORIGINAL_CATEGORY'),
				'batch[category]',
				JHtml::_('select.options', JHtml::_('category.options', 'com_recipemanager'), 'value', 'text')
			);
		}
	}
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_('COM_RECIPEMANAGER_RECIPES'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_recipemanager/assets/css/recipes.css", (RecipemanagerHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
	}
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.name' => JText::_('COM_RECIPEMANAGER_RECIPE_NAME_LABEL'),
