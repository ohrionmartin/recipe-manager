...
/**
 * Recipemanager component helper.
 */
abstract class RecipemanagerHelper
{
	/**
	 * Composer Switch
	 * 
	...
	{
		$manifestUrl = JPATH_ADMINISTRATOR."/components/com_recipemanager/recipemanager.xml";
	...
	public static function addSubmenu($submenu)
	{
		...
		JHtmlSidebar::addEntry(JText::_('COM_RECIPEMANAGER_SUBMENU_DASHBOARD'), 'index.php?option=com_recipemanager&view=recipemanager', $submenu === 'recipemanager');
		if ($user->authorise('ingredient.access', 'com_recipemanager') && $user->authorise('ingredient.submenu', 'com_recipemanager'))
		{
			JHtmlSidebar::addEntry(JText::_('COM_RECIPEMANAGER_SUBMENU_INGREDIENTS'), 'index.php?option=com_recipemanager&view=ingredients', $submenu === 'ingredients');
		}
		if ($user->authorise('recipe.access', 'com_recipemanager') && $user->authorise('recipe.submenu', 'com_recipemanager'))
		{
			JHtmlSidebar::addEntry(JText::_('COM_RECIPEMANAGER_SUBMENU_RECIPES'), 'index.php?option=com_recipemanager&view=recipes', $submenu === 'recipes');
			JHtmlSidebar::addEntry(JText::_('COM_RECIPEMANAGER_RECIPE_RECIPES_CATEGORIES'), 'index.php?option=com_categories&view=categories&extension=com_recipemanager', $submenu === 'categories');
		}
	}
	...
	*/
	 ...
	 *
	 */
	public static function getVar($table, $where = null, $whereString = 'user', $what = 'id', $operator = '=', $main = 'recipemanager')
	{
		if(!$where)
		{
	 ...
	 *
	 */
	public static function getVars($table, $where = null, $whereString = 'user', $what = 'id', $operator = 'IN', $main = 'recipemanager', $unique = true)
	{
		if(!$where)
		{
			$where = JFactory::getUser()->id;
	...
	public static function isPublished($id,$type)
	{
		...
		$query->from('#__recipemanager_'.$type.' AS a');
	 ...
	 * 
	 */
	public static function getActions($view, &$record = null, $views = null, $target = null, $component = 'recipemanager', $user = 'null')
	{
	 ...
	 */
	public static function getModel($name, $path = JPATH_COMPONENT_ADMINISTRATOR, $Component = 'Recipemanager', $config = array())
	{
		// fix the name
	...
	public static function setAsset($id, $table, $inherit = true)
	{
		...
		$parent->loadByName('com_recipemanager');
		
		$parentId = $parent->id;
		$name     = 'com_recipemanager.'.$table.'.'.$id;
		$title    = '';
		...
		else
		{
			...
			$rules = self::getDefaultAssetRules('com_recipemanager', $table, $inherit);
			...
			else
			{
				...
				return JFactory::getDbo()->updateObject('#__recipemanager_'.$table, $object, 'id');
			}
		}
		return false;
	}
	...
	public static function transliterate($string)
	{
		...
		{
			// get global value
			self::$langTag = JComponentHelper::getParams('com_recipemanager')->get('language', 'en-GB');
		}
