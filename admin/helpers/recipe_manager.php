...
/**
 * Recipe_manager component helper.
 */
abstract class Recipe_managerHelper
{
	/**
	 * Composer Switch
	 * 
	...
	{
		$manifestUrl = JPATH_ADMINISTRATOR."/components/com_recipe_manager/recipe_manager.xml";
	...
	public static function addSubmenu($submenu)
	{
		...
		JHtmlSidebar::addEntry(JText::_('COM_RECIPE_MANAGER_SUBMENU_DASHBOARD'), 'index.php?option=com_recipe_manager&view=recipe_manager', $submenu === 'recipe_manager');
		if ($user->authorise('ingredient.access', 'com_recipe_manager') && $user->authorise('ingredient.submenu', 'com_recipe_manager'))
		{
			JHtmlSidebar::addEntry(JText::_('COM_RECIPE_MANAGER_SUBMENU_INGREDIENTS'), 'index.php?option=com_recipe_manager&view=ingredients', $submenu === 'ingredients');
		}
		if ($user->authorise('recipe.access', 'com_recipe_manager') && $user->authorise('recipe.submenu', 'com_recipe_manager'))
		{
			JHtmlSidebar::addEntry(JText::_('COM_RECIPE_MANAGER_SUBMENU_RECIPES'), 'index.php?option=com_recipe_manager&view=recipes', $submenu === 'recipes');
			JHtmlSidebar::addEntry(JText::_('COM_RECIPE_MANAGER_RECIPE_RECIPES_CATEGORIES'), 'index.php?option=com_categories&view=categories&extension=com_recipe_manager', $submenu === 'categories');
		}
	}
	...
	*/
	public static function xls($rows, $fileName = null, $title = null, $subjectTab = null, $creator = 'Joomla Component Builder', $description = null, $category = null,$keywords = null, $modified = null)
	{
		...
		if (($size = self::checkArray($rows)) !== false)
		{
			...
			// [Interpretation 1691] Based on data size we adapt the behaviour.
			$xls_mode = 1;
			if ($size > 3000)
			{
				$xls_mode = 3;
			...
			// [Interpretation 1703] Set active sheet and get it.
	...
	*/
	 ...
	 *
	 */
	public static function getVar($table, $where = null, $whereString = 'user', $what = 'id', $operator = '=', $main = 'recipe_manager')
	{
		if(!$where)
		{
	 ...
	 *
	 */
	public static function getVars($table, $where = null, $whereString = 'user', $what = 'id', $operator = 'IN', $main = 'recipe_manager', $unique = true)
	{
		if(!$where)
		{
			$where = JFactory::getUser()->id;
	...
	public static function isPublished($id,$type)
	{
		...
		$query->from('#__recipe_manager_'.$type.' AS a');
	 ...
	 * 
	 */
	public static function getActions($view, &$record = null, $views = null, $target = null, $component = 'recipe_manager', $user = 'null')
	{
	 ...
	 */
	public static function getModel($name, $path = JPATH_COMPONENT_ADMINISTRATOR, $Component = 'Recipe_manager', $config = array())
	{
		// fix the name
	...
	public static function setAsset($id, $table, $inherit = true)
	{
		...
		$parent->loadByName('com_recipe_manager');
		
		$parentId = $parent->id;
		$name     = 'com_recipe_manager.'.$table.'.'.$id;
		$title    = '';
		...
		else
		{
			...
			$rules = self::getDefaultAssetRules('com_recipe_manager', $table, $inherit);
			...
			else
			{
				...
				return JFactory::getDbo()->updateObject('#__recipe_manager_'.$table, $object, 'id');
			}
		}
		return false;
	}
	...
	public static function transliterate($string)
	{
		...
		{
			// get global value
			self::$langTag = JComponentHelper::getParams('com_recipe_manager')->get('language', 'en-GB');
		}
